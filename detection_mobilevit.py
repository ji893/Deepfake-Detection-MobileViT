...
class MobileViTDetector(nn.Module):
    """MobileViT 기반 딥페이크 탐지 모델 (이진 분류)"""
    def __init__(self, num_classes=1, pretrained=True, model_name='mobilevit_s'):
        super(MobileViTDetector, self).__init__()
        
        # MobileViT 백본 로드 (timm 라이브러리 사용)
        # mobilevit_s: Small 버전 (5.6M params)
        # mobilevit_xs: Extra-Small 버전 (2.3M params)
        # mobilevit_xxs: Extra-Extra-Small 버전 (1.3M params)
        ...
        print(f"MobileViT feature dimension: {feature_dim}")
        
        # Classification head (이진 분류)
...
def train_model_kfold(data_splits, dataset_size, model_name='mobilevit_s'):
    """K-Fold 교차 검증을 사용한 MobileViT 모델 학습 및 평가"""
    print(f"\n{'='*60}")
    print(f"데이터셋 크기: {dataset_size}장으로 K-Fold {CONFIG['k_fold']} 교차 검증 시작")
    print(f"모델: MobileViT ({model_name})")
    print(f"{'='*60}\n")
    ...
    for fold, (train_idx, fold_val_idx) in enumerate(kfold.split(train_paths)):
        ...
        # MobileViT 모델 초기화
        model = MobileViTDetector(num_classes=1, pretrained=True, model_name=model_name).to(device)
        ...
        for epoch in range(CONFIG['epochs']):
            ...
            if val_acc > best_val_acc:
                ...
                fold_best_model_state = model.state_dict().copy()
    ...
    # 원래 분리한 Val 세트 평가 (Best 모델 사용)
    print(f"\n{'='*60}")
    print("원래 분리한 Val 세트 평가 (Best 모델 사용)")
    print(f"{'='*60}\n")
    ...
    # Best 모델 로드
    model = MobileViTDetector(num_classes=1, pretrained=False, model_name=model_name).to(device)
...
def train_model(datasets, dataset_size, model_name='mobilevit_s'):
    """MobileViT 모델 학습 및 평가 (K-Fold + 8:1:1 분할)"""
    print(f"\n{'='*60}")
    ...
    # K-Fold 교차 검증으로 학습
    results = train_model_kfold(data_splits, dataset_size, model_name)
    
    return results


def plot_results(all_results, model_name='mobilevit_s'):
    """결과 시각화"""
    if not all_results:
        print("시각화할 결과가 없습니다.")
        return
    ...
    fig.suptitle(f'MobileViT ({model_name}): K-Fold Cross-Validation + Source-wise Performance', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig('results/mobilevit_kfold_source_wise_comparison.png', dpi=300, bbox_inches='tight')
    print("\n시각화 결과가 저장되었습니다: results/mobilevit_kfold_source_wise_comparison.png")


def plot_training_history(all_results, model_name='mobilevit_s'):
    """학습 히스토리 시각화"""
    if not all_results:
        return
    
    for size, results in all_results.items():
        fig, axes = plt.subplots(2, CONFIG['k_fold'], figsize=(5*CONFIG['k_fold'], 8))
        fig.suptitle(f'MobileViT ({model_name}) - {size} Training History', fontsize=14, fontweight='bold')
        
        for fold_idx, fold_result in enumerate(results['fold_results']):
            history = fold_result['train_history']
            
            # Loss plot
            axes[0, fold_idx].plot(history['loss'], label='Train Loss', color='blue')
            axes[0, fold_idx].plot(history['val_loss'], label='Val Loss', color='red')
            axes[0, fold_idx].set_title(f'Fold {fold_idx + 1} - Loss')
            axes[0, fold_idx].set_xlabel('Epoch')
            axes[0, fold_idx].set_ylabel('Loss')
            axes[0, fold_idx].legend()
            axes[0, fold_idx].grid(True, alpha=0.3)
            
            # Accuracy plot
            axes[1, fold_idx].plot(history['acc'], label='Train Acc', color='blue')
            axes[1, fold_idx].plot(history['val_acc'], label='Val Acc', color='red')
            axes[1, fold_idx].set_title(f'Fold {fold_idx + 1} - Accuracy')
        ...
        plt.tight_layout()
        plt.savefig(f'results/mobilevit_{size}_training_history.png', dpi=300, bbox_inches='tight')
        print(f"학습 히스토리 저장됨: results/mobilevit_{size}_training_history.png")


def main():
    """메인 함수"""
    print("="*60)
    print("MobileViT 딥페이크 탐지 학습")
    print("(K-Fold 5 교차 검증 + 8:1:1 분할)")
    print("="*60)
    ...
    print(f"  Data Augmentation: Minimal (Flip, Rotation)")
    
    # MobileViT 모델 선택
    model_name = 'mobilevit_s'  # 'mobilevit_s', 'mobilevit_xs', 'mobilevit_xxs'
    print(f"  Model: MobileViT ({model_name})")
    ...
    for size in dataset_sizes:
        ...
        results = train_model(datasets, size, model_name)
    ...
    # 결과 저장
    df.to_csv(output_dir / "mobilevit_kfold_source_wise_results.csv", index=False, encoding='utf-8-sig')
    
    # 시각화
    plot_results(all_results, model_name)
    plot_training_history(all_results, model_name)
    
    print(f"\n결과가 {output_dir}에 저장되었습니다.")
    print("  - mobilevit_kfold_source_wise_results.csv")
    print("  - mobilevit_kfold_source_wise_comparison.png")
    print("  - mobilevit_*장_training_history.png")
